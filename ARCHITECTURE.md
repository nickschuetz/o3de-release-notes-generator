...
- `POINTRELEASE_CONTAINER_PATTERNS` - Regex list (subset of cherry-pick patterns specialised for *containers*: commits whose bodies enumerate bundled PRs via the `(#NNNN)` convention). Used by `extract_pointrelease_containers()` when writing the audit sidecar. A title match is sufficient but no longer necessary: a commit whose title matches `CHERRY_PICK_PATTERNS` and whose title or body references another PR number is a container too, because GitHub's squash keeps the picked subjects in the body however the container was named (`Cherrypick fixes to 26100 (first pass)`, o3de/o3de#20091, names its destination and matched no container pattern). A merge commit (`Merge pull request #N`) whose subject or body matches a cherry-pick pattern is listed as a merge-commit container, with the bundled PRs read from the commits it merged in (first parent..second parent). The container's own number is the *last* `(#NNNN)` in the title, since GitHub appends the squash number after any the title already carried.
...
**Multi-repo support:** The `parse_repo_path_mappings()` function resolves per-repo local clone paths. Each repo can have its own clone via `--repo-path owner/repo=/path`, with `--default-repo-path` as the fallback. `parse_repo_ref_mappings()` does the same for git refs via `--repo-from-ref` / `--repo-to-ref`, which exist because release lines are not always tagged uniformly. For most of the 26.10.0 cycle `o3de/o3de` carried `2605.0` while `o3de/o3de-extras` did not, and a single global ref would abort the whole run on the untagged repo; extras was tagged `2605.0` on 2026-05-27, so the override is no longer needed for that pair but the mechanism still is. All mapping flags use `action='extend'`, so both `--flag a=1 b=2` and `--flag a=1 --flag b=2` accumulate.

**Preflight:** `verify_refs_exist()` resolves every `(repo, from-ref)` and `(repo, to-ref)` pair with `git rev-parse --verify` before any git log or API work, and reports each failure with the remedy. This converts a mid-run abort, potentially after minutes of GitHub calls, into an actionable error at second zero. `warn_on_shallow_clones()` runs alongside it and flags any clone with truncated history, because a shallow clone does not fail: `git log A..B` answers from whatever it holds. During the 26.10.0 cycle a routine `git fetch` re-fetched the branch refs under shallow rules and cut them to roughly 20 commits each while the tag retained its full 26,947, reducing the window from about 200 PRs to 18 with nothing in the output to distinguish that from a quiet release. It warns rather than aborting, since a shallow clone can still be deep enough to cover the window.
...
**Point-release awareness and audit:** When `--from-ref` parses as a point-release tag with a non-zero patch (e.g. `2510.2`), `_emit_point_release_awareness_log()` runs `git merge-base` against the major sibling tag (`2510.0`) and `--to-ref`; when the merge-bases match, it logs a single `INFO` line so future runs don't relearn the equivalence. `_maybe_write_pointrelease_audit()` then iterates per repo, calls `extract_pointrelease_containers()` to find cherry-pick container PRs between the major and point-release tags, parses bundled PR numbers from each container's commit body, and writes a `<output-md-stem>_pointrelease_audit.md` sidecar with a ✓/✗ checklist for every bundled fix vs the rendered report. Suppressed via `--no-pointrelease-audit`. The same machinery also runs when `--to-ref` names a `stabilization/NNNNN` branch (`is_stabilization_ref()`), scanning `--from-ref..--to-ref` and writing `<stem>_cherrypick_audit.md`. That case is the one that carries the most cherry-picks, and it was previously unaudited: a container that is merged keeps each picked commit's original `(#NNNN)` so the fix reports itself, but a squashed container carries only its own number, is filtered out as a cherry-pick, and takes every bundled fix with it. Bundled PRs resolve to one of four states, and the fourth matters as much as the others: a PR present in an `--exclude-json` source is `○ already reported`, not `✗ missing`. The first live run without that state produced 18 crosses and an "action required" verdict of which 13 were simply prior-release content. Merge-commit containers are listed with a note and their bundled fixes show ✓, so the harmless outcome is confirmed rather than inferred from silence; #20006/#19998 reads that way on the live 26.10.0 window.
