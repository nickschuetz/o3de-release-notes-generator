...
- Subprocess stderr is scrubbed for classic (`ghp_/gho_/ghu_/ghs_/ghr_`) and fine-grained (`github_pat_`) GitHub token shapes before logging.
- PR titles and body-derived descriptions are sanitized for markdown special characters **and HTML tag openers**, exactly once; PR bodies are capped at 64KB before extraction. LLM narrative output passes the same filter.
- LLM summary command runtime is bounded (`--summary-timeout`, default 300s).
- File writes are atomic (`tempfile.mkstemp()` + `fsync` + `os.replace()`) and preserve the destination's permission bits.
- Every `git` / `gh` call site converts subprocess timeouts and missing binaries into handled errors rather than aborting mid-run.
